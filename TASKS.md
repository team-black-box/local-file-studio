...
- Dependency alerts, Dependabot security updates, standard secret scanning, and push protection are enabled. At this checkpoint code scanning was not configured; see the later 2026-08-27 CodeQL checkpoint for the owner-authorized change.
...
- Standard secret scanning and push protection are enabled. The later 2026-08-27 checkpoint records the completed default CodeQL evaluation and alert triage.
...
### Default CodeQL checkpoint — 2026-08-27

- The owner explicitly authorized GitHub-managed CodeQL default setup. It is configured with the default query suite and remote threat model for GitHub Actions and JavaScript/TypeScript, using standard runners on relevant branch/pull-request events plus a weekly schedule. No repository workflow file, workflow write permission, paid feature, required status check, deployment, or application runtime dependency was added.
- The first analysis examined exact public `main` commit `86d4e9f248f162ea8a2a45c3d2d1a05543ed0c21`. Both the JavaScript/TypeScript and GitHub Actions jobs passed in [run 32974849978](https://github.com/team-black-box/local-file-studio/actions/runs/32974849978). Findings create code-scanning alerts and pull-request annotations; default setup does not automatically open ordinary GitHub issues.
- The initial analysis opened 12 alerts. Alert #1 was a test-only `String.includes` branch used to select a known Strict OOXML fixture, not a URL trust, redirect, navigation, or request decision; the triage change replaces it with explicit fixture cases. Alerts #4–#12 are React image-preview `src` attributes fed only by browser-created `blob:` URLs for locally selected or locally generated image blobs, with matching URL cleanup; no file-controlled string is interpreted as HTML. These are false-positive DOM-XSS reports.
- Alerts #2 and #3 are deliberate inert parsing boundaries for untrusted HTML and SVG. The HTML path removes executable/embedded/form/style/template elements, event and presentation attributes, remote-resource attributes and CSS URLs, retains only bounded raster data images, disables controls, and inserts only the sanitized tree into an isolated capture shadow root. The SVG path removes executable/embedded elements, event handlers, external references/imports/CSS URLs, and serializes the sanitized SVG into a new local blob decoded only as an image. Existing focused tests cover URL and attribute rejection contracts. The analyzer reports the initial inert parse and cannot establish the post-parse sanitization; maintainer review classified these as false positives rather than suppressing the rules globally.
- All 12 initial alerts were dismissed with specific maintainer comments: #1 as used only in tests and #2–#12 as false positives. Alert dismissal is mutable GitHub state and does not replace source review, sanitizer tests, CSP, dependency/provenance checks, or `bun run verify`. Recheck default-setup configuration and open alerts live before releases and after security-setting changes.
...
- Public-source launch work is complete. The default CodeQL evaluation is complete as recorded above; the exhaustive production QA matrix, Vercel Git-author access, and deferred capabilities remain ongoing work rather than hidden launch claims.
