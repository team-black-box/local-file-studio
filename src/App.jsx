...
function getTextAnnotationPlan(settings, info) {
  if (info.state === "idle") return { valid: false, message: "Add one PDF to place the text note." };
  if (info.state === "loading") return { valid: false, message: "Reading the PDF pages locally…" };
  if (info.state === "error") return { valid: false, message: info.message };
  try {
    return { valid: true, ...createPdfTextAnnotationPlan(settings, info.pageCount), message: "" };
  } catch (error) {
    return { valid: false, message: error?.message || "Check the text, page, size, and position before continuing." };
  }
}
...
function PdfTextAnnotationCanvas({ document, pageIndex, text, fontSize, x, y, onMove }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      page = await document.getPage(pageIndex + 1);
      const base = page.getViewport({ scale: 1 });
      const scale = Math.min(1.7, 720 / base.width, 920 / base.height);
      const viewport = page.getViewport({ scale });
      const canvas = canvasRef.current;
      if (!canvas || cancelled) return;
      canvas.width = Math.max(1, Math.ceil(viewport.width));
  ...
  const positionFromEvent = (event) => {
    ...
    return {
      x: Math.round(Math.max(0, Math.min(100, ((event.clientX - bounds.left) / bounds.width) * 100)) * 10) / 10,
      y: Math.round(Math.max(0, Math.min(100, ((event.clientY - bounds.top) / bounds.height) * 100)) * 10) / 10,
    };
  };
  ...
  const beginMove = (event) => {
    ...
    try { frameRef.current?.setPointerCapture(event.pointerId); } catch { /* Pointer may already be captured. */ }
  };
  ...
  const moveWithKeyboard = (event) => {
    ...
    onMove(Math.max(0, Math.min(100, Number(x) + deltaX)), Math.max(0, Math.min(100, Number(y) + deltaY)));
  };

  return (
    <div className={`text-annotation-page-shell ${renderState}`}>
      <div
        ref={frameRef}
        className="text-annotation-page-frame"
        style={{ aspectRatio }}
...
function EditPdfControls({ settings, settingsList, onChange, onApply, info, plan }) {
  const [pageIndex, setPageIndex] = useState(0);
  const [pageWindowStart, setPageWindowStart] = useState(0);
  const pageRailRef = useRef(null);
  const pendingRailAlignmentRef = useRef(null);
  ...
  const pagesPerWindow = 6;
  const pageCount = info.pageCount || 0;
  const textSetting = settingsList.find((setting) => setting.key === "text");
  ...
  useEffect(() => {
    if (!pageCount || pageIndex < pageCount) return;
    setPageIndex(pageCount - 1);
  }, [pageCount, pageIndex]);

  useEffect(() => {
    if (settings.scope !== "single") return;
    const targetIndex = Number(settings.targetPage) - 1;
    if (!Number.isInteger(targetIndex) || targetIndex < 0 || targetIndex >= pageCount || targetIndex === pageIndex) return;
    setPageIndex(targetIndex);
    setPageWindowStart(Math.floor(targetIndex / pagesPerWindow) * pagesPerWindow);
  }, [pageCount, pageIndex, settings.scope, settings.targetPage]);
  ...
  const choosePreviewPage = (index) => {
    setPageIndex(index);
    if (settings.scope === "single") onChange("targetPage", index + 1);
  };

  const chooseScope = (scope) => {
    onApply({ scope, ...(scope === "single" ? { targetPage: pageIndex + 1 } : {}) });
  };

  const updatePosition = (x, y) => onApply({ x, y });
  const resetPosition = () => onApply({ x: PDF_TEXT_ANNOTATION_DEFAULTS.x, y: PDF_TEXT_ANNOTATION_DEFAULTS.y });
  const ready = info.state === "ready" && info.document;

  return (
    <section className="text-annotation-editor" aria-labelledby="text-annotation-editor-title">
      <div className="text-annotation-editor-heading">
        <span><strong id="text-annotation-editor-title">Add a text note</strong><small>This adds new text—it does not change existing PDF text.</small></span>
        {plan.valid && <b aria-live="polite">{plan.affectedPageCount.toLocaleString()} {plan.affectedPageCount === 1 ? "page" : "pages"}</b>}
      </div>

      <div className="text-annotation-copy">
        <label htmlFor="edit-pdf-text"><strong>{textSetting.label}</strong><small>{String(settings.text || "").length.toLocaleString()} / {Number(textSetting.maxLength).toLocaleString()}</small></label>
        <textarea id="edit-pdf-text" rows="3" maxLength={textSetting.maxLength} value={settings.text} onChange={(event) => onChange("text", event.target.value)} />
      </div>

      <fieldset className="text-annotation-scope">
        <legend>Where should it appear?</legend>
        <div>
          {Object.values(PDF_TEXT_ANNOTATION_SCOPES).map((scope) => (
            <button type="button" key={scope.value} className={settings.scope === scope.value ? "selected" : ""} aria-pressed={settings.scope === scope.value} onClick={() => chooseScope(scope.value)}>
              <span>{scope.value === "all" ? <StackIcon size={19} weight="duotone" /> : <FilePdfIcon size={19} weight="duotone" />}</span>
      ...
      {!ready ? <PdfPageSourceStatus info={info} /> : (
        <>
          <div className="text-annotation-canvas-heading">
            <span><strong>Page {pageIndex + 1} of {pageCount}</strong><small>{settings.scope === "single" ? `The note will be added only to page ${Number(settings.targetPage).toLocaleString()}.` : "Move the note here; the same position is used on every page."}</small></span>
            <button type="button" onClick={resetPosition}><ClockCounterClockwiseIcon size={15} />Reset position</button>
          </div>
          <PdfTextAnnotationCanvas document={info.document} pageIndex={pageIndex} text={settings.text} fontSize={settings.fontSize} x={settings.x} y={settings.y} onMove={updatePosition} />
          <p className="text-annotation-tip"><ArrowsOutIcon size={16} weight="fill" aria-hidden="true" /><span>Drag the note or click anywhere on the page. Focus the note and use arrow keys for 1% steps; hold Shift for 5%.</span></p>

          <div className="text-annotation-page-strip-heading">
            <span><strong>{settings.scope === "single" ? "Choose the output page" : "Preview another page"}</strong><small>Scroll sideways with a trackpad or swipe.</small></span>
            <span className="text-annotation-page-strip-actions">
            {settings.scope === "single" && (
              <label><span>Page</span><input type="number" min="1" max={pageCount} step="1" value={settings.targetPage} aria-label={`Output page from 1 to ${pageCount}`} onChange={(event) => onChange("targetPage", event.target.value)} /><small>of {pageCount}</small></label>
            )}
            {pageCount > pagesPerWindow && (
              <span className="text-annotation-page-window-controls">
                <button type="button" onClick={() => showPageWindow(pageWindowStart - pagesPerWindow, "end")} disabled={pageWindowStart === 0} aria-label="Show previous PDF pages"><ArrowLeftIcon size={14} /></button>
                <b aria-live="polite">{pageWindowStart + 1}–{Math.min(pageWindowStart + pagesPerWindow, pageCount)} of {pageCount}</b>
                <button type="button" onClick={() => showPageWindow(pageWindowStart + pagesPerWindow)} disabled={pageWindowStart + pagesPerWindow >= pageCount} aria-label="Show next PDF pages"><ArrowRightIcon size={14} /></button>
              </span>
          ...
          <div ref={pageRailRef} className="text-annotation-page-strip" role="group" aria-label="PDF pages for text placement" onWheel={movePageRailHorizontally}>
            {visiblePages.map((index) => {
              const selectedForOutput = settings.scope === "all" || Number(settings.targetPage) === index + 1;
              return (
                <button type="button" key={index} className={`${index === pageIndex ? "selected" : ""} ${selectedForOutput ? "included" : ""}`} aria-pressed={index === pageIndex} aria-label={`Page ${index + 1}${selectedForOutput ? ", receives the text note" : ", preview only"}`} onClick={() => choosePreviewPage(index)}>
                  <span><PdfPageThumbnail document={info.document} pageIndex={index} />{selectedForOutput && <b aria-hidden="true"><CheckCircleIcon size={13} weight="fill" /></b>}</span>
                  <small>Page {index + 1}</small>
                </button>
              );
            })}
      ...
      <details className="text-annotation-exact-controls">
        ...
        <p>Measured from the page’s top-left corner. The output keeps the whole note inside safe page margins.</p>
        <div>
      ...
      <div className={`text-annotation-plan ${plan.valid ? "ready" : "waiting"}`} role="status" aria-live="polite">
        ...
        <span>{plan.valid ? `${plan.scopeLabel} will receive this ${plan.fontSize.toLocaleString()} pt note.` : plan.message}</span>
      </div>
    </section>
  );
}
...
function TextAnnotationResultSummary({ result }) {
  const outcome = result?.textAnnotationOutcome;
  if (!outcome) return null;
  const scope = outcome.scope === "single"
    ? `Page ${outcome.targetPage.toLocaleString()} only`
    : `All ${outcome.pageCount.toLocaleString()} pages`;
  return (
...
function FaceBlurResultSummary({ result }) {
  const batch = result?.faceBlurBatchOutcome;
  const outcome = result?.faceBlurOutcome;
  if (!batch && !outcome) return null;

  if (batch) {
    const detection = [
      batch.detectedFiles ? `${batch.detectedFiles.toLocaleString()} auto-detected` : "",
      batch.fallbackFiles ? `${batch.fallbackFiles.toLocaleString()} reviewed fallback` : "",
    ].filter(Boolean).join(" · ");
    return (
...
function ImageRotationResultSummary({ result }) {
  const batch = result?.imageRotationBatchOutcome;
  const outcome = result?.imageRotationOutcome;
  if (!batch && !outcome) return null;

  if (batch) {
    const rotation = getImageRotation(batch.angle);
    return (
      <div className="image-rotation-result-summary" role="status" aria-live="polite">
        <span><ArrowClockwiseIcon size={23} weight="duotone" aria-hidden="true" /></span>
        <div><strong>{batch.fileCount.toLocaleString()} images rotated</strong><small>{rotation.label} · {batch.formats.map((format) => format.toUpperCase()).join(" / ")} · original shapes preserved</small></div>
        <b>{formatBytes(result.size)} ZIP</b>
      </div>
    );
  }

  const rotation = getImageRotation(outcome.angle);
  return (
    <div className="image-rotation-result-summary" role="status" aria-live="polite">
      <span><ArrowClockwiseIcon size={23} weight="duotone" aria-hidden="true" /></span>
      <div><strong>{outcome.sourceWidth.toLocaleString()} × {outcome.sourceHeight.toLocaleString()} <ArrowRightIcon size={15} aria-hidden="true" /> {outcome.width.toLocaleString()} × {outcome.height.toLocaleString()} px</strong><small>{rotation.label} · {rotation.hint} · {outcome.format.toUpperCase()}</small></div>
      <b>{formatBytes(outcome.outputBytes)}</b>
    </div>
  );
}
...
function useFaceBlurPreview(file, settings, enabled, tool) {
...
const faceBlurPresetIcons = Object.freeze({
  12: FeatherIcon,
  24: ScalesIcon,
  40: EyeSlashIcon,
});
...
function FaceBlurControls({ files, settings, setting, onChange, onMove, preview }) {
  ...
  const batch = files.length > 1;
  const outcome = preview.state === "ready" ? preview.result : null;
  const fallback = outcome?.mode === "centered-fallback";
  const dragRef = useRef(false);

  const moveFallback = (event) => {
    if (!fallback) return;
    const bounds = event.currentTarget.getBoundingClientRect();
    const focusX = Math.max(10, Math.min(90, Math.round(((event.clientX - bounds.left) / bounds.width) * 100)));
    const focusY = Math.max(10, Math.min(90, Math.round(((event.clientY - bounds.top) / bounds.height) * 100)));
    onMove(focusX, focusY);
  };

  const fallbackReason = outcome?.fallbackReason === "unavailable"
    ? "This browser does not provide native face detection."
    : outcome?.fallbackReason === "detection-error"
      ? "Native face detection could not inspect this image."
      : "No face was found automatically in this image.";

  return (
    <section className="face-blur-controls" aria-labelledby="face-blur-preview-title">
      <div className="face-blur-preview-heading">
        <span><strong id="face-blur-preview-title">Review the privacy area</strong><small>{file ? batch ? `${file.name}, first of ${files.length.toLocaleString()} images` : file.name : "Add an image to inspect it locally"}</small></span>
        <EyeSlashIcon size={18} weight="duotone" aria-hidden="true" />
      </div>

      {preview.state === "ready" && outcome ? (
        <figure className="face-blur-preview-figure">
          {fallback ? (
            <button
              type="button"
        ...
        <div className="face-blur-preview-state error" role="alert"><WarningCircleIcon size={20} weight="fill" aria-hidden="true" /><span><strong>Preview needs attention</strong><small>{preview.message}</small></span></div>
      ) : (
        <div className="face-blur-preview-state"><EyeSlashIcon size={21} weight="duotone" aria-hidden="true" /><span><strong>No image selected</strong><small>The exact blur area will appear here for review.</small></span></div>
      )}
      ...
      <fieldset className="face-blur-strength-choices">
        <legend>Choose the blur strength</legend>
        <div>{FACE_BLUR_STRENGTHS.map((option) => {
          const Icon = faceBlurPresetIcons[option.value];
          const selected = Number(settings.strength) === option.value;
          return <button type="button" key={option.value} className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => onChange("strength", option.value)}><Icon size={19} weight="duotone" aria-hidden="true" /><span><strong>{option.label}</strong><small>{option.hint}</small></span></button>;
        })}</div>
      </fieldset>

      <label className="face-blur-strength-range" htmlFor="face-blur-strength">
        <span><strong>Fine-tune</strong><output>{getFaceBlurStrength(settings.strength).toLocaleString()} px</output></span>
        <input id="face-blur-strength" type="range" min={setting.min} max={setting.max} step={setting.step} value={settings.strength} onChange={(event) => onChange("strength", Number(event.target.value))} />
...
function useImageRotationPreview(file, angle, enabled, tool) {
  const normalizedAngle = Number(angle || IMAGE_ROTATIONS[0].value);
  const [preview, setPreview] = useState({ state: "idle", file: null, angle: normalizedAngle, outputUrl: "", result: null, message: "" });

  useEffect(() => {
    if (!file || !enabled) {
      setPreview({ state: "idle", file: null, angle: normalizedAngle, outputUrl: "", result: null, message: "" });
      return undefined;
    }

    let cancelled = false;
    let outputUrl = "";
    setPreview({ state: "loading", file, angle: normalizedAngle, outputUrl: "", result: null, message: "" });
    const timer = window.setTimeout(() => {
      (async () => {
        await preflightToolFiles(tool, [file], { angle: normalizedAngle });
        const { createImageRotationPreview } = await import("./lib/image-processors.js");
        const result = await createImageRotationPreview(file, normalizedAngle);
        outputUrl = URL.createObjectURL(result.blob);
        if (cancelled) {
          URL.revokeObjectURL(outputUrl);
          outputUrl = "";
        ...
        setPreview({ state: "ready", file, angle: normalizedAngle, outputUrl, result, message: "" });
      })().catch((error) => {
        if (outputUrl) URL.revokeObjectURL(outputUrl);
        outputUrl = "";
        if (!cancelled) setPreview({ state: "error", file, angle: normalizedAngle, outputUrl: "", result: null, message: toFriendlyResourceError(error, "Rotate Image")?.message || "This rotation preview could not be created safely." });
      });
    }, 120);
  ...
  }, [enabled, file, normalizedAngle, tool]);

  return preview;
}

const imageRotationIcons = Object.freeze({
  90: ArrowClockwiseIcon,
  180: RepeatIcon,
  270: ClockCounterClockwiseIcon,
});

function ImageRotationControls({ files, angle, onChange, preview }) {
  const file = files[0];
  const batch = files.length > 1;
  const outcome = preview.state === "ready" ? preview.result : null;
  ...
  return (
    <section className="image-rotation-controls" aria-labelledby="image-rotation-preview-title">
      <div className="image-rotation-preview-heading">
        <span><strong id="image-rotation-preview-title">Live rotation</strong><small>{file ? batch ? `${file.name}, first of ${files.length.toLocaleString()} images` : file.name : "Add an image to see its new direction"}</small></span>
        <EyeIcon size={17} aria-hidden="true" />
      </div>

      <fieldset className="image-rotation-choices">
        <legend>Choose a direction</legend>
        <div>{IMAGE_ROTATIONS.map((option) => {
          const Icon = imageRotationIcons[option.value];
          const selected = option.value === rotation.value;
          return <button type="button" key={option.value} className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => onChange(option.value)}><Icon size={21} weight={selected ? "bold" : "regular"} aria-hidden="true" /><span><strong>{option.label}</strong><small>{option.hint}</small></span></button>;
        })}</div>
      </fieldset>

      {preview.state === "ready" && outcome ? (
        <figure className="image-rotation-preview-figure">
          <div><img src={preview.outputUrl} alt={`Preview of ${file.name} rotated ${rotation.hint.toLowerCase()}`} /></div>
          <figcaption><span><strong>{outcome.sourceWidth.toLocaleString()} × {outcome.sourceHeight.toLocaleString()} <ArrowRightIcon size={14} aria-hidden="true" /> {outcome.width.toLocaleString()} × {outcome.height.toLocaleString()} px</strong><small>{rotation.label} · the full export keeps every source pixel.</small></span><b>{compressionInputFormat(file).toUpperCase()}</b></figcaption>
        </figure>
      ) : preview.state === "loading" ? (
        <div className="image-rotation-preview-state" role="status"><SpinnerGapIcon size={20} className="spin" aria-hidden="true" /><span><strong>Turning the first image locally…</strong><small>The preview is bounded so every direction stays responsive.</small></span></div>
      ) : preview.state === "error" ? (
        <div className="image-rotation-preview-state error" role="alert"><WarningCircleIcon size={20} weight="fill" aria-hidden="true" /><span><strong>Preview needs attention</strong><small>{preview.message}</small></span></div>
      ) : (
        <div className="image-rotation-preview-state"><ArrowClockwiseIcon size={21} weight="duotone" aria-hidden="true" /><span><strong>No image selected</strong><small>The first image will turn here before you export the batch.</small></span></div>
      )}

      {batch && preview.state === "ready" && <p className="image-rotation-batch-note"><StackIcon size={16} weight="duotone" aria-hidden="true" /><span><strong>First-image preview, one direction for the batch.</strong> Every image turns {rotation.hint.toLowerCase()} and keeps its own dimensions and format.</span></p>}
      <p className="image-rotation-private-note"><ShieldCheckIcon size={16} weight="fill" aria-hidden="true" /><span>The originals stay untouched. Preview and full-resolution export run entirely in this tab; re-encoding removes image metadata.</span></p>
    </section>
  );
}
...
function GenericToolWorkbench({ tool, onClose, onComplete }) {
  ...
  const faceBlurPreview = useFaceBlurPreview(files[0], settings, tool.slug === "blur-face", tool);
  ...
  const imageRotationPreview = useImageRotationPreview(files[0], settings.angle, tool.slug === "rotate-image", tool);
  ...
  const faceBlurPreviewMatches = faceBlurPreview.state === "ready"
  ...
  const addFiles = (incoming) => {
    ...
    if (validation.accepted.length) {
      ...
      if (tool.slug === "edit-pdf") setSettings((current) => ({ ...current, targetPage: PDF_TEXT_ANNOTATION_DEFAULTS.targetPage }));
  ...
  const faceBlurReady = tool.slug !== "blur-face" || !hasRequiredInput || faceBlurPreviewMatches;
  ...
  const imageRotationReady = tool.slug !== "rotate-image" || !hasRequiredInput || imageRotationPreviewMatches;
  ...
  const processHint = !hasRequiredInput
    ...
    : tool.slug === "blur-face" && faceBlurPreview.file === files[0] && faceBlurPreview.state === "loading"
      ? "Detecting and previewing the first-image privacy area locally."
    : tool.slug === "blur-face" && faceBlurPreview.file === files[0] && faceBlurPreview.state === "error"
      ? faceBlurPreview.message
    ...
    : tool.slug === "rotate-image" && imageRotationPreview.file === files[0] && imageRotationPreview.state === "loading"
      ? "Turning the first-image preview locally."
    : tool.slug === "rotate-image" && imageRotationPreview.file === files[0] && imageRotationPreview.state === "error"
      ? imageRotationPreview.message
  ...
  const process = async () => {
    ...
    try {
      ...
      if (tool.slug === "blur-face" && faceBlurPreviewMatches) {
        processOptions.faceBlurPreview = {
          file: files[0],
          result: faceBlurPreview.result,
        };
      }
  ...
  const processButtonLabel = tool.slug === "merge-pdf" && mergePdfPreviewMatches
    ...
    : tool.slug === "blur-face" && hasRequiredInput && faceBlurPreviewMatches
      ? files.length === 1
        ? faceBlurPreview.result.mode === "detected"
          ? `Blur ${faceBlurPreview.result.regionCount.toLocaleString()} ${faceBlurPreview.result.regionCount === 1 ? "face" : "faces"}`
          : "Blur reviewed area"
        : `Blur faces in ${files.length.toLocaleString()} images`
    ...
    : tool.slug === "meme-generator" && hasRequiredInput && imageMemePreviewMatches
      ? "Create meme"
    : tool.slug === "rotate-image" && hasRequiredInput && imageRotationPreviewMatches
      ? files.length === 1 ? getImageRotation(settings.angle).label : `${getImageRotation(settings.angle).label} · ${files.length.toLocaleString()} images`
        ...
        <div className={`workbench-body ${usesPagePicker ? "page-picker-body" : ""} ${tool.slug === "split-pdf" ? "split-planner-body" : tool.slug === "remove-pdf-pages" ? "remove-pages-planner-body" : tool.slug === "extract-pdf-pages" ? "extract-pages-planner-body" : tool.slug === "organize-pdf" ? "organize-pages-planner-body" : tool.slug === "redact-pdf" ? "redact-planner-body" : tool.slug === "edit-pdf" ? "text-annotation-body" : tool.slug === "compare-pdf" ? "compare-planner-body" : tool.slug === "translate-pdf" ? "translation-preview-body" : tool.slug === "pdf-to-jpg" ? "pdf-jpg-preview-body" : tool.slug === "compress-image" ? "image-compression-preview-body" : tool.slug === "resize-image" ? "image-resize-preview-body" : tool.slug === "upscale-image" ? "image-upscale-preview-body" : tool.slug === "remove-image-background" ? "background-removal-preview-body" : tool.slug === "blur-face" ? "face-blur-preview-body" : tool.slug === "watermark-image" ? "image-watermark-preview-body" : tool.slug === "meme-generator" ? "image-meme-preview-body" : tool.slug === "rotate-image" ? "image-rotation-preview-body" : tool.slug === "crop-image" ? "image-crop-preview-body" : tool.slug === "convert-from-jpg" ? "image-gif-preview-body" : tool.slug === "photo-editor" ? "photo-editor-preview-body" : tool.slug === "html-to-image" ? "html-image-preview-body" : usesPdfOfficeTextPreview ? "pdf-office-text-preview-body" : ""}`}>
          <section className="file-stage" aria-label="Files">
            ...
            {results.length > 0 && !inlineReaderTools.has(tool.slug) && (
              <div className="results-card">
                ...
                {tool.slug === "blur-face" && <FaceBlurResultSummary result={results[0]} />}
                ...
                {tool.slug === "rotate-image" && <ImageRotationResultSummary result={results[0]} />}
          ...
          <aside className={`settings-panel ${usesStickySettings ? "page-picker-settings-panel" : ""}`} aria-label="Tool settings">
            ...
            {tool.slug === "translate-pdf" ? (
              ...
              <ImageRotationControls files={files} angle={settings.angle} onChange={(value) => updateSetting("angle", value)} preview={imageRotationPreview} />
            ...
            <div className="process-action-stack">
              ...
              {tool.slug === "rotate-image" && imageRotationPreviewMatches && status !== "processing" && (
                <strong className="split-ready-count" aria-live="polite">{files.length.toLocaleString()} {files.length === 1 ? "image" : "images"} · {getImageRotation(settings.angle).label}</strong>
              )}
