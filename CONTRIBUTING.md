...
Pull requests should be small enough to review, explain why the behavior is safe, and include tests or documentation for user-visible changes. A passing build is necessary but does not replace manual checks for visual output, browser compatibility, offline operation, malformed input, or resource cleanup.
...
A maintainer may ask you to amend or re-sign commits that lack a valid `Signed-off-by` trailer. DCO sign-off is distinct from cryptographic Git commit signing; repository rules can require both independently.

## Review and release

Maintainers merge changes only after required CI, DCO, review, and applicable production-preview checks pass. Merging does not promise an immediate public release. The production and rollback process is documented in [docs/RELEASE_OPERATIONS.md](docs/RELEASE_OPERATIONS.md).
