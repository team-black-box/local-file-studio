...
Every workbench shows its exact file-count, per-file, combined-size, page/pixel, and result limits next to the file picker. These deterministic limits bound browser memory use; files are validated before local engines allocate large canvases, page buffers, or ZIP archives. Size labels use binary units: 1 MB in the interface means 1,048,576 bytes (1 MiB).

- Most structural PDF tools accept one file up to 100 MB and 500 pages, including Split, Extract, Organize, Forms, Unlock, and Protect. Repair accepts 100 MB/300 pages; Unlock and Protect retain 1,024-character passwords. Merge accepts 2–20 PDFs, 100 MB each, 120 MB combined, 300 pages per file, and 500 pages combined. Compare accepts exactly two 50 MB PDFs, up to 250 pages each/400 combined and 2,000,000 extracted characters, with 25,000 lines per file/40,000 combined, 2,000 line edits, a 3-second diff budget, and a 4-second hard worker stop.
- Raster PDF tools retain separate render budgets: Compress accepts the standard 100 MB PDF input with 150 pages/150 MP rendered per job; Redact and PDF-to-JPG are 50 MB/100 pages/150 MP; OCR is 30 MB/25 pages/40 MP. Redact allows 200 areas per PDF, at most 50 on one page, with 65,536 characters of bounded area data. Each rendered page is capped at 12–16 MP and a 6,000–8,192 px edge.
...
Limits do not represent upload quotas: document bytes never leave the browser. The former 50 MB Merge and Compress caps were conservative memory safeguards, not PDF-format restrictions. Structural tools and Compress now share a 100 MB input budget. Compress retains its page and decoded-pixel guards, since compressed file size does not determine render memory. Merge retains its 120 MB combined budget to leave room below the 128 MB result ceiling; merging scanned pages usually preserves their compressed image bytes rather than reducing them. Page, generated-file, and result limits still apply independently, and output expansion can still cause a safe failure. Other raster tools, OCR, overlay, comparison, and text-processing tools retain their tighter budgets. Actual headroom can be lower when other tabs use substantial memory or a browser has a stricter canvas implementation; acceptance does not guarantee that every device can finish a job.
