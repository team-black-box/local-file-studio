...
Blur Face shows the exact privacy area on a bounded first-image preview before export. When the browser provides native face detection, every detected region is outlined for review; otherwise a clearly labelled centered oval can be dragged, clicked, or moved with keyboard-accessible controls onto the face. Light, Balanced, and Strong choices stay synchronized with the exact 8–48 px control, and the reviewed first-image region is reused for full-resolution output. Batch images are inspected independently and use the same relative fallback position when detection is unavailable. Detection is browser-dependent and cannot guarantee anonymity, so the interface requires visual review; orange guides are preview-only, processing remains local, and re-encoding removes image metadata.
